...
- LLM access is provided via a self-hosted **liteLLM proxy server** (OpenAI-compatible `POST /v1/chat/completions` endpoint, e.g. `http://localhost:4000`), not the liteLLM Python library called in-process. The FastAPI backend calls this proxy over HTTP, authenticated with a proxy virtual key (`Authorization: Bearer <key>`), requesting the model via the proxy-side alias `genailab-maas-Opus-4.6` (the proxy's configured name for Claude Opus 4.6 — not the raw Anthropic model ID `claude-opus-4-6`). The proxy itself holds the real upstream credentials and is run/configured as a separate process alongside the FastAPI backend — no new secrets-management infrastructure beyond existing environment-variable configuration, but this is an added deployable component beyond the FastAPI app itself (a deliberate, explicit exception to Principle III's "no external services," justified by the desire to keep the AI provider swappable/proxyable without changing backend code). Responses follow the OpenAI chat-completions response shape (`choices[0].message.content`), including proxy-specific usage fields (`cache_read_input_tokens`, `inference_geo`, etc.) that the backend does not need to consume beyond the message content.
