...
type IdentityCard = { name: string; initials: string; role: string; activity: string; score: number; color: string; description: string };
...
function IdentityProfiles() {
  ...
  useEffect(() => {
    api
      .get<{ identities: IdentityCard[] }>("/identities")
      .then((response) => {
        setIdentities(response.data.identities);
        setSelected((current) => current ?? response.data.identities[0] ?? null);
      })
      .catch(() => toast.error("Could not load identities", { description: "The backend is unreachable. Try again shortly." }));
  }, []);

  useEffect(() => {
    if (!selected) return;
    api
      .get<{ events: ActivityRow[] }>(`/identities/${encodeURIComponent(selected.name)}/timeline`)
...
function Reports({ onExport }: { onExport: () => void }) {
  ...
  const prepare = (title: string) => {
    api
      .post(`/reports/${encodeURIComponent(title)}/prepare`)
      .then(() => {
