...
const initialAlerts: AlertRecord[] = [
  {
    id: "ALT-2841",
    severity: "Critical",
    title: "Impossible travel + privilege pivot",
    entity: "Aisha Rahman",
    role: "Cloud Engineer",
    ...
    score: 96,
    time: "08:42 UTC",
    description: "A privileged console session originated from a new anonymizing network shortly after a successful session in Singapore.",
    signals: ["New ASN", "11,240 km in 39 min", "AssumeRole to Admin", "Outside peer baseline"],
    baseline: "Aisha typically signs in from Singapore on weekdays between 08:00–19:00 UTC and has not used this ASN in the past 90 days.",
    recommended: "Require step-up authentication and review the created access key before allowing further privilege changes.",
  },
  {
    id: "ALT-2840",
    severity: "High",
    title: "Unusual bulk secret retrieval",
    entity: "build-runner-prod",
    ...
    score: 89,
    time: "08:31 UTC",
    description: "The deployment identity read 28 secrets outside its normal release window.",
    signals: ["5.6× normal volume", "New secret path", "Weekend pattern", "No change ticket"],
    baseline: "This workload normally retrieves 4–6 scoped secrets only during the 02:00 UTC release window.",
    recommended: "Compare the task definition with the latest approved deployment and rotate any unneeded credentials.",
  },
  {
    id: "ALT-2838",
    severity: "High",
    title: "Dormant account reactivated",
    entity: "noah.williams",
    role: "Data Analyst",
    source: "45.83.64.12",
    region: "Toronto, CA",
    service: "Azure AD",
    score: 81,
    time: "08:04 UTC",
    description: "An account inactive for 63 days signed in and enumerated production storage containers.",
    signals: ["Dormant 63 days", "First managed device", "Storage enumeration"],
    baseline: "Noah’s identity has no recorded activity since the contractor offboarding review in June.",
    recommended: "Validate current employment status and terminate active refresh tokens if access is no longer required.",
  },
  {
    id: "ALT-2837",
    severity: "Medium",
    title: "New API geography",
    entity: "maya.chen",
    role: "Platform Engineer",
    source: "89.187.161.75",
    region: "Bucharest, RO",
    service: "GitHub Enterprise",
    score: 67,
    time: "07:46 UTC",
    description: "A personal access token performed repository administration from an unseen country.",
    signals: ["New country", "PAT usage", "Repository settings"],
    baseline: "Maya’s prior 30-day access is limited to London and Amsterdam through managed devices.",
    recommended: "Confirm travel, inspect the token’s scope, and revoke it if it was not intentionally used.",
  },
  {
    id: "ALT-2834",
    severity: "Medium",
    title: "Policy edit outside change window",
    entity: "samir.patel",
    ...
    score: 61,
    time: "07:15 UTC",
    description: "An S3 bucket policy change was performed shortly after a denied service-control-policy action.",
    signals: ["Denied action sequence", "Out-of-window change", "Sensitive resource"],
    baseline: "Samir’s policy changes usually occur through CI with linked change records, not the console.",
    recommended: "Compare the policy version with the approved infrastructure-as-code change and document the exception.",
  },
];

const accessTrend = [
  { label: "00", events: 116, anomalies: 3 },
  { label: "03", events: 94, anomalies: 2 },
  { label: "06", events: 126, anomalies: 4 },
  { label: "09", events: 226, anomalies: 11 },
  { label: "12", events: 284, anomalies: 9 },
  { label: "15", events: 249, anomalies: 6 },
  { label: "18", events: 314, anomalies: 13 },
  { label: "21", events: 180, anomalies: 5 },
];

const serviceRisk = [
  { name: "IAM", risk: 88, events: 410 },
  { name: "Secrets", risk: 71, events: 276 },
  { name: "Azure AD", risk: 63, events: 224 },
  { name: "GitHub", risk: 44, events: 186 },
  { name: "GCP", risk: 29, events: 101 },
];
...
export default function Home() {
  ...
  const activeInvestigations = alerts.filter((alert) => alert.status === "In progress").length;
  ...
  const simulateAnomaly = () => {
    const created: AlertRecord = {
      id: `ALT-${2842 + alerts.length}`,
      severity: "High",
      title: "Synthetic anomalous token replay",
      entity: "ops-automation-bot",
      role: "Service identity",
      source: "203.0.113.58",
      region: "São Paulo, BR",
      service: "GitHub Enterprise",
      score: 84,
      time: "Now",
      description: "A clearly labeled demo event replayed a service token from an unseen geography and triggered a scoped repository action.",
      signals: ["Synthetic demo event", "New country", "Token replay pattern", "Unexpected repository scope"],
      baseline: "The automation identity usually acts from eu-west-1 and touches only deployment repositories during scheduled pipelines.",
      recommended: "Use the demo finding to explain how behavioral context turns one API call into an analyst-ready decision.",
    };
        ...
        <div className={workspaceView === "command" ? "mx-auto max-w-[1600px] px-5 py-6 sm:px-8 sm:py-8" : "hidden"}>
          ...
          <section id="alert-queue" className="mt-6 grid gap-6 2xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,0.65fr)]">
            <div className="glass-panel overflow-hidden rounded-2xl">
              ...
              <div className="overflow-x-auto"><table className="w-full min-w-[730px] text-left"><thead><tr className="mono border-b border-white/[0.07] text-[9px] uppercase tracking-[0.12em] text-slate-600"><th className="px-5 py-3 font-medium sm:px-6">Activity to check</th><th className="px-3 py-3 font-medium">Who or what</th><th className="px-3 py-3 font-medium">What we noticed</th><th className="px-3 py-3 font-medium">Priority score</th><th className="px-5 py-3 text-right font-medium sm:px-6">When</th></tr></thead><tbody>{filteredAlerts.map((alert) => <tr onClick={() => setSelected(alert)} key={alert.id} className={`queue-row cursor-pointer border-b border-white/[0.06] border-l-[3px] ${severityTone[alert.severity].border} bg-transparent hover:bg-white/[0.035]`}><td className="px-5 py-4 sm:px-6"><div className="flex items-center gap-3"><SeverityPill severity={alert.severity} /><div><p className="text-sm font-medium text-slate-100">{alert.title}</p><p className="mono mt-1 text-[10px] text-slate-600">{alert.id} · {alert.service}{alert.status && <span className="font-semibold text-[#73ecd9]"> · IN PROGRESS</span>}</p></div></div></td><td className="px-3 py-4"><p className="text-xs font-medium text-slate-300">{alert.entity}</p><p className="mt-1 text-[10px] text-slate-600">{alert.role}</p></td><td className="px-3 py-4"><div className="flex max-w-[180px] flex-wrap gap-1">{alert.signals.slice(0, 2).map((signal) => <span key={signal} className="rounded bg-white/[0.055] px-1.5 py-1 text-[9px] text-slate-400">{signal}</span>)}</div></td><td className="px-3 py-4"><RiskTrace value={alert.score} severity={alert.severity} /></td><td className="px-5 py-4 text-right sm:px-6"><p className="mono text-[10px] text-slate-400">{alert.time}</p><ArrowUpRight className="ml-auto mt-1 size-3 text-slate-600" /></td></tr>)}</tbody></table></div>
            ...
            <div className="space-y-6"><div className="glass-panel rounded-2xl p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="mono text-[10px] uppercase tracking-[0.15em] text-[#77eddb]">04 / model rationale</p><h3 className="mt-1 text-lg font-semibold tracking-tight text-slate-100">Why this was elevated</h3></div><FileWarning className="size-5 text-[#f5ae45]" /></div><p className="mt-4 text-sm leading-6 text-slate-400">The strongest signal is not a single unusual event. It is the sequence: an unfamiliar network, an impossible geography transition, and an immediate privilege change.</p><div className="mt-5 space-y-3">{[{ label: "Identity deviation", score: 96, tone: "#ff5962" }, { label: "Access sequence", score: 91, tone: "#f5ae45" }, { label: "Asset sensitivity", score: 82, tone: "#94a4b5" }, { label: "Peer-group variance", score: 76, tone: "#39e0c5" }].map((signal) => <div key={signal.label}><div className="mb-1.5 flex justify-between text-xs"><span className="text-slate-400">{signal.label}</span><span className="mono text-slate-300">{signal.score}</span></div><SegmentedTrace value={signal.score} color={signal.tone} segments={14} /></div>)}</div><button onClick={() => setSelected(alerts[0])} className="mt-6 inline-flex items-center gap-2 text-xs font-semibold text-[#78eedc] hover:text-[#c0fff4]">Open top evidence <ArrowUpRight className="size-3.5" /></button></div>
